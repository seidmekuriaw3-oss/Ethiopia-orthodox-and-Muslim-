...
## Quran Amharic cache — 429 rate limits, not a real gap

- `database/quran_amharic_loader.py` lazy-fetches per-surah from `api.alquran.cloud` and caches to `quran_amharic_cache.json`; it had only 5/114 surahs cached because most users never trigger every surah, not because the API/data is broken.
- Hitting all 114 surahs back-to-back in a tight loop gets some (~7) `429 Too Many Requests`; those surahs silently stay uncached (`get_surah_amharic` returns `None` on failure) and show no Amharic in the reader until retried.
- **Fix:** re-request only the failed surah numbers with a short delay between attempts (a few seconds) — they succeed on retry. Verified all 114/114 surahs cached and serving Amharic via `/api/quran/surah/<n>/bilingual` after this. Restart the app workflow afterward so its in-memory `_mem_cache` picks up the refreshed disk cache.
